...
from decoders.utils import k_means_anchors
...
class LSTM_anchor(nn.Module):
    def __init__(self, args) -> None:
        ...
        self.prob_op = nn.Linear(self.embedding_size, self.num_modes)
        self.relu = nn.ReLU()
    ...
    def generate_anchors(self, tr):
        ...
        :return anchors: [num_modes, target_len, op_dim]
        """
    ...
    def forward(self, hist, hidden):
        ...
        #use hidden state to predict [1, batch_size, op_dim]
        outputs = self.linear2(hist_dec[0])
        log_probs = self.log_softmax(self.prob_op(embedded.squeeze(0)))
        # probs = probs.squeeze(dim=0)

        return outputs, hist_dec, log_probs
...
with trange(n_epochs) as tr:
    ...
    for it in tr:
        ...
        for i, data in enumerate(trDataloader):
            ...
            # decoder_input, decoder_hidden = lstm_decoder.module.init_multimodal(decoder_input0, decoder_hidden0)
            decoder_outputs_tr = torch.zeros(fut_tr.shape[0], args["num_modes"], fut_tr.shape[1], 2).to(device)
            probs_tr = torch.zeros(fut_tr.shape[0],  fut_tr.shape[1], args["num_modes"],).to(device)
            _,_,log_probs= lstm_decoder(decoder_input, decoder_hidden)
            #decode hidden state in future trajectory
            for di in range(args['target_len']):
                decoder_output, decoder_hidden, _= lstm_decoder(decoder_input, decoder_hidden)
                anchor_di = anchors[:,di,:].unsqueeze(1)
                decoder_output = decoder_output.to(device) + anchor_di.to(device) #[num_modes, batch_size, op_dim]
                decoder_outputs_tr[di, :,:, :] = decoder_output
                probs_tr[di,:,:] = torch.exp(log_probs)
                #update loss
                l2_norm = torch.linalg.vector_norm(
                    decoder_output[:,:,:2].to(device) - fut_tr[di,:,:].unsqueeze(0).to(device),
                    dim=(-2,-1))
                best_mode = l2_norm.argmin(dim=0)
                decoder_output_best = decoder_output[best_mode, :, :2]

                loss_reg = criterion(decoder_output_best.to(device), fut_tr[di, :, :].to(device))
                loss_tr += loss_reg
                #multimodal class
                soft_target = F.softmax(-l2_norm, dim=0).t().detach()
                loss_cls = cls_loss(torch.exp(log_probs), soft_target)
                loss_tr += loss_cls
                #use own predictions as inputs at next step
                decoder_input = decoder_output_best.unsqueeze(0)
        ...
        with torch.no_grad():
            for i, data in enumerate(valDataloader):
                ...
                #Initialize multimodal decoder
                decoder_outputs_val = torch.zeros(fut_val.shape[0], args["num_modes"], fut_val.shape[1], 2).to(device)
                probs_val = torch.zeros(fut_val.shape[0],  fut_val.shape[1], args["num_modes"],).to(device)
                #decode hidden state in future trajectory
                for di in range(args['target_len']):
                    decoder_output, decoder_hidden, log_probs_val = lstm_decoder(decoder_input, decoder_hidden)
                    anchor_di = anchors[:,di,:].unsqueeze(1)
                    decoder_output = decoder_output.to(device) + anchor_di.to(device)
                    decoder_outputs_val[di, :, :, :] = decoder_output
                    probs_val[di, :, :] = torch.exp(log_probs_val)
                
                    #update loss
                    l2_norm_val = torch.linalg.vector_norm(
                        decoder_output[:,:,:2].to(device) - fut_val[di,:,:].unsqueeze(0).to(device),
                        dim=(-2,-1))
                    best_mode_val = l2_norm_val.argmin(dim=0)
                    decoder_output_best_val = decoder_output[best_mode_val, :, :2]

                    loss_val += criterion(decoder_output_best_val.to(device), fut_val[di, :, :].to(device))
                    #multimodal class
                    # soft_target_val = F.softmax(-l2_norm_val, dim=0).t().detach()
                    # loss_cls_val = cls_loss(torch.exp(log_probs_val), soft_target_val)
                    # loss_val += loss_cls_val
                    
                    #use own predictions as inputs at next step
                    decoder_input = decoder_output_best_val.unsqueeze(0)
